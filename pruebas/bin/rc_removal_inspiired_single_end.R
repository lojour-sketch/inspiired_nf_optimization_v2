...
#we only keep reads that are longer thatn 0 in both reads. as we don't want them paired, we just remove them from both files
keep_indices1 <- (width(trimmed_reads1) > 0)
keep_indices2 <- (width(trimmed_reads2) > 0)


# Create new ShortRead objects. we remove the ids of the reads that are empty
output_fq1 <- ShortReadQ(trimmed_reads1[keep_indices1], trimmed_qual1[keep_indices1], id(fq1)[keep_indices1])
output_fq2 <- ShortReadQ(trimmed_reads2[keep_indices2], trimmed_qual2[keep_indices2], id(fq2)[keep_indices2])

# Check lengths

  paste0("Final lengths: R1=", length(output_fq1), " R2=", length(output_fq2))
