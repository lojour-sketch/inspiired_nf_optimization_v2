process FINDVECTOR_local {
    ...
    output:
    tuple val(meta), path("${meta}.R1_vector_removed.fastq.gz"), path("${meta}.R2_vector_removed.fastq.gz"), path("${meta}.vector_read_names.txt"), emit: ch_vector_removed

    script:
    """
    find_vector_single_end.R "${meta}" "${read1}" "${read2}" "${primer}${ltrbit}" "${vector_fasta}"
