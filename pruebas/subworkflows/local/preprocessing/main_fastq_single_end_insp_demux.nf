...
/////////////////////////// DISCLAIMER ///////////////////////////
//WE ARE NOT USING THE SINGLE_END MAIN.NF FILES, BECAUSE I FOUND OUT THAT INSPIIRED DOES TAKES THE INTERSECTION BETWEEN R1 AND R2 AFTER TRIMMING PRIMER + LTR AND ALSO AFTER TRIMMING OVERREADING (REVERSE COMPLEMENT)
include { DEMUXING_FASTQ_inspiired_like } from '../../../modules/local/demuxing_fastq_inspiired_like/main'
...
include { FINDVECTOR_local } from '../../../modules/local/find_vector_local/main_single_end'
include { SHORTREMOVE_local } from '../../../modules/local/short_seq_removal_local/main_single_end'
include { CREATE_KEYS_local } from '../../../modules/local/create_keys_local/main'
...
workflow PREPROCESSING_wfl {
    ...
    ch_demux_fastq = ch_index2_all
        .map { samples, indices -> tuple(samples, indices, params.FASTQfolderDir, file(params.demuxSampleSheet)) }
    
    ch_demux_fastq.view { "Demuxing input channel content: ${it}" }

    DEMUXING_FASTQ_inspiired_like(ch_demux_fastq)
    DEMUXING_FASTQ_inspiired_like.out.fastq.set { ch_demux_fastq }
    ...
    ///////////////////////// Extracting UMI from reads and adding it to headers /////////////////////////
        ...
        ch_reads_by_sample = ch_demux_fastq
            ...
            .map { file ->
                ...
                if (name.endsWith('_R1.fastq.gz')) {
                    sample_name = name.substring(0, name.length() - '_R1.fastq.gz'.length())
                } else if (name.endsWith('_R2.fastq.gz')) {
                    sample_name = name.substring(0, name.length() - '_R2.fastq.gz'.length())
                }
    ...
    ///////////////////////// Filtering FASTQs by quality and removing Ns from sequences /////////////////////////
        ...
        //as we are doing single end, we have [meta, [r1]] and [meta, [r2]] in each channel
    ...
    ///////////////////////// Remove short sequences /////////////////////////
        ...
        def short_removed_ch = SHORTREMOVE_local.out.reads


    ///////////////////////// Create keys /////////////////////////

        //for the input of create_keys, we need to join ch_shortr_removed with ch_vector removed,
        //because we need the reads and also the vqNames

        ch_vector_removed
            .join(short_removed_ch)
            .set { ch_create_keys_input }

        CREATE_KEYS_local(ch_create_keys_input)
        def keys_ch = CREATE_KEYS_local.out.keys
