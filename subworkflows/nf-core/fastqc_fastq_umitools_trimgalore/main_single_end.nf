...
workflow FASTQCANDTRIM_wfl {
    ...
    if (with_umi && !skip_umi_extract) {
        ...
        //separate r1 and r2 for single end
    umi_reads_se = umi_reads
        .flatMap { meta, reads ->
            def (r1, r2) = reads
            def meta_r1 = meta.clone() + ['single_end': true, read_group: 'R1', id: meta.id + "_R1"]
            def meta_r2 = meta.clone() + ['single_end': false, read_group: 'R2', id: meta.id + "_R2"]
            [[meta_r1, [r1]], [meta_r2, [r2]]]
        }

    trim_reads = umi_reads_se
    ...
    if (!skip_trimming) {
        TRIMGALORE(umi_reads_se)
        trim_reads = TRIMGALORE.out.trimmed_reads
    ...
    //we join the r1 and r2 reads back together
    trim_reads
        .map { meta, reads ->
            def original_id = meta.id.replaceAll(/_R[12]$/, '')
            def read_group = meta.read_group
            [original_id, read_group, reads]
        }
        .groupTuple(by: 0)
        .map { original_id, read_groups, reads_list ->
            def sorted = [read_groups, reads_list].transpose().sort { it[0] }
            def joined_reads = sorted.collect { it[1][0] }
...
//for debugging
//      umi_reads_se must have this type of data: D83_CART_d0_S7, single_end=false, reads=[/beegfs/home/lrenteria/inspiired_nf/work/5e/664d31fe7f6f2e8a6303fe4b0ac925/D83_CART_d0_S7.umi_R1.fastq.gz, /beegfs/home/lrenteria/inspiired_nf/work/5e/664d31fe7f6f2e8a6303fe4b0ac925/D83_CART_d0_S7.umi_R2.fastq.gz]
//          umi_reads_se.view { "TRIMGALORE input tuple: ${it[0].id}, single_end=${it[0].single_end}, reads=${it[1]}" }